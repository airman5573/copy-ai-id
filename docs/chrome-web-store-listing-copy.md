...
Updated: 2026-06-23 KST — fallback target metadata wording

## Positioning / Single Purpose

Copy AI ID helps developers and QA reviewers work with rendered pages using a `data-ai-id`-first workflow. It opens a full-screen editor on the current tab so the user can inspect the DOM layout tree, preview responsive breakpoints in an iframe, highlight exact DOM nodes, write local notes for stable `data-ai-id` targets or generated fallback targets, and copy the note bundle for QA, development, or AI-assisted UI editing workflows.
...
- copy note text for QA, development, or AI editing workflows
...
- Iframe preview with six responsive breakpoint widths
...
- Always-visible note panel with explicit copy action
