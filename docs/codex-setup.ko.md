...
이 가이드는 Copy AI ID `0.1.16`용입니다. 확장 프로그램이 만드는 프롬프트는 자체 manifest 버전에서 `v0.1.16`을 계산하므로 변경되는 `main` 브랜치를 설치하지 않습니다. 버전 고정 소스 URL은 일치하는 GitHub 릴리스/tag가 게시되면 열립니다.
...
1. 이 가이드와 확장 빌드에 일치하는 [Copy AI ID v0.1.16 릴리스](https://github.com/airman5573/copy-ai-id/releases/tag/v0.1.16)를 엽니다.
2. `copy-ai-id-codex-companion-0.1.16-macos.zip`을 받습니다. 이 단계에서 Chrome Web Store ZIP을 받지 마세요.
