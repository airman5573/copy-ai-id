...
3. Use **Copy prompt**, paste the prompt into Codex, and let it finish. The prompt points Codex to the release-matched [`setup-copy-ai-id-codex` Skill](https://github.com/airman5573/copy-ai-id/tree/v0.1.15/skills/setup-copy-ai-id-codex) in this repository.
...
This guide targets Copy AI ID `0.1.15`. The extension-generated prompt derives `v0.1.15` from its own manifest version so it never installs the moving `main` branch. The versioned source URL resolves when the matching GitHub release/tag is published.
...
1. Open the [Copy AI ID v0.1.15 release](https://github.com/airman5573/copy-ai-id/releases/tag/v0.1.15), which matches this guide and extension build.
2. Download `copy-ai-id-codex-companion-0.1.15-macos.zip`. Do not download the Chrome Web Store ZIP for this step.
