...
Updated: 2026-06-23 KST — fallback target metadata inventory

## Summary for Chrome Web Store privacy fields

Copy AI ID is a developer/QA browser extension for inspecting rendered DOM targets, preferring stable `data-ai-id` attributes, viewing a DOM layout tree, previewing the current page at responsive iframe widths, writing local note text, and copying that note text after an explicit user action. For elements without usable `data-ai-id`, it can generate fallback target metadata locally.

The extension does not sell user data, does not use advertising IDs, does not run analytics, does not include a native host, and does not send notes or page content to a remote service. Current editor state is runtime-only in the extension/page context.
...
| Clipboard writes | `src/content/clipboard/clipboard.ts`, `src/editor/notebook/copy.ts` | User-created notebook text, selected `data-ai-id` references or fallback target metadata, and selected suffixes | `navigator.clipboard.writeText` after explicit Copy / Shift + Enter; no manifest clipboard permission | Clipboard write on explicit user action |
...
> Copy AI ID reads rendered page DOM information, including `data-ai-id` attributes and locally generated fallback metadata such as selectors, selector type/reliability, DOM paths, concise nearby text, class tokens, and accessibility context, only while the user enables the editor. This data is used locally to show the layout tree, preview selection outlines, keyboard traversal across layout-tree nodes, and note panel. Copy AI ID does not sell user data, use advertising IDs, run analytics, include a native host, or send page content or notes to a remote server. Notebook text is copied to the clipboard only after the user explicitly clicks Copy or presses Shift + Enter.
