...
- *Overlays/highlights*: `overlay.ts` (hover outline, drop indicator, rAF repositioning), `highlight.ts` (hover/selection brain: posts `targetHighlighted`, pins the quick-action toolbar on click, Space → `targetReferenceRequested`), `box-model.ts` + `box-model-geometry.ts` (DevTools-style margin/border/padding/content layers), `visual-box-highlight.ts`.
