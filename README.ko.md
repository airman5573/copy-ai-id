...
Copy AI ID는 렌더링된 페이지를 위한 `data-ai-id` 우선 Chrome 확장 에디터입니다. 현재 탭에서 켜면 전체 화면 Shadow DOM 에디터가 열리고, 레이아웃 트리·반응형 iframe 미리보기·노트 패널로 AI 코딩 도구에 전달할 UI 수정 노트를 만들 수 있습니다. 선택한 요소는 노트북에 compact한 `el-N` chip으로 들어가며, `data-ai-id`가 있는 요소는 안정적인 reference로 매핑되고 없는 요소도 생성된 fallback target으로 선택할 수 있습니다.
...
3. 현재 탭 위에 전체 화면 에디터가 열립니다.
   ...
   - **가운데 — 미리보기:** 현재 URL에 `copy-ai-id-preview=1` 쿼리 마커를 붙인 iframe 미리보기와 6개 breakpoint 버튼, 확대/축소, 맞춤/초기화 컨트롤을 제공합니다.
   - **오른쪽 — 노트 패널:** 선택한 안정적인 `data-ai-id` target 또는 생성된 fallback target에 대한 Lexical 기반 노트 draft를 작성합니다. Target은 compact한 `el-N` chip으로 표시됩니다.
4. 미리보기, 레이아웃 트리, 키보드로 DOM 노드를 선택합니다.
5. **Space**를 누르면 `el-1` 같은 notebook chip이 추가되고 노트 패널에 포커스됩니다. Copy AI ID는 chip target에 `data-ai-id`를 먼저 사용합니다. 선택한 노드에 사용할 수 있는 `data-ai-id`가 없으면 긴 selector/path/context 텍스트를 에디터에 노출하지 않고 fallback metadata를 chip에 저장합니다. Chip을 클릭하면 연결된 미리보기 요소가 다시 선택/강조됩니다. Chip 번호는 삭제 후에도 재번호 매김하지 않으므로 draft에 `el-1`, `el-3`, `el-4`가 함께 있을 수 있습니다.
6. **Shift + Enter**를 누르거나 **복사** 버튼을 클릭하면 `## Requests`, `## Targets`, `## Rules` 섹션으로 정리된 AI 친화적 Markdown이 클립보드에 복사됩니다. Inline chip은 읽기 쉬운 `@el-N` mention으로 표시되고, fallback target은 selector/path/context 정보를 함께 제공합니다.
7. 툴바 닫기 버튼, **Esc**, 또는 **Shift + Z + Space**로 에디터를 끕니다.
...
| **Space** | 선택 노드를 compact한 `el-N` chip으로 추가/포커스합니다. Chip은 안정적인 `data-ai-id` target을 우선 사용하고, 없으면 가능한 경우 생성된 fallback target을 저장합니다. |
...
Copy AI ID는 이제 에디터 전용입니다. Codex 사이드패널, native messaging host, AI 채팅, 히스토리, 설정 화면, prompt 전송, analytics, 원격 AI 처리는 포함하지 않습니다. 노트는 사용자가 명시적으로 **복사**를 누르거나 **Shift + Enter**를 눌렀을 때만 클립보드에 기록됩니다.
